...
**Sanctioned rewrite — supersession compaction.** Shards are append-only with one narrow exception: when ingest re-reads a record whose *value* changed but whose schema + source + time-frame match a line already on disk — a **cumulative total that grew** (Samsung Health exposes the day's steps as one whole-day record that climbs) or an **in-place correction** — the day's shard is compacted, dropping the stale line(s) and rewriting the file **atomically** (temp + rename, like `manifest.json`). This is the durable, write-side form of the §4.3 last-ingested-wins rule; without it the shard would accumulate a snapshot per refresh. It fires only on an actual supersession — a run that supersedes nothing takes the plain-append path — and it is **deterministic** (keep the newest per `(schema, source, time-frame)`), so two devices compacting the same shard converge rather than diverge. `manifest.json` and `profile.json` are likewise rewritten in place. The read-path last-ingested-wins remains the invariant every reader must apply, both as a safety net and because a shard can still briefly hold un-compacted snapshots between refreshes. Two guards keep the rewrite safe: it **refuses to compact a shard that holds an unparseable line** (a torn append `readRange` would skip) — rewriting from the parsed lines alone would erase it — and falls back to a plain append instead; and it is **single-writer-only**, picking the newest by local ingest order, not a source-recency signal (the health store gives none for cumulative totals — Samsung's `source_creation_date_time` is just the day key). Recency-aware, conflict-safe compaction across devices is part of the **Phase 8** sync/merge work; until then a second device must not compact a shard the first is still writing.
